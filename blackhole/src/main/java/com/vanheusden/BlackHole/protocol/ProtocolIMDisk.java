...
public class ProtocolIMDisk extends Protocol {
	...
	public ProtocolIMDisk(SectorMapper sm, int lun, Storage storage,
			MirrorManager mirrorMan, SnapshotManager snapMan,
			MyClientSocket mcs, long size, boolean assumeBarriers)
			throws IOException {
		this.sm = sm;
		this.lun = lun;
	...
	static public long x86ToJavaLong(byte[] in) {
		...
		return ((((((((long) (in[7] & 0xff) << 8) | (long) (in[6] & 0xff)) << 8 | (long) (in[5] & 0xff)) << 8 | (long) (in[4] & 0xff)) << 8 | (long) (in[3] & 0xff)) << 8 | (long) (in[2] & 0xff)) << 8 | (long) (in[1] & 0xff)) << 8
				| (long) (in[0] & 0xff);
	}
	...
	protected void msgWrite(long offset, long len) throws IOException,
			InterruptedException, VHException, SQLException,
	...
	protected void msgRead(long offset, long len) throws IOException,
	...
	public void run() {
		...
		try {
			...
			Log.log(LogLevel.LOG_WARN, "protocolIMDisk run() socket exception "
					+ se);
		} catch (IOException ie) {
			Log.log(LogLevel.LOG_WARN, "protocolIMDisk run() IO exception "
					+ ie);
		} catch (VHException vhe) {
			Log.log(LogLevel.LOG_WARN, "protocolIMDisk run() text exception "
					+ vhe);
		} catch (Exception e) { // OK
			Log.log(LogLevel.LOG_WARN, "protocolIMDisk run() other exception "
					+ e);
			Log.showException(e);
