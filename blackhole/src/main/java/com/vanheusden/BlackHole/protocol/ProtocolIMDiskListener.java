...
public class ProtocolIMDiskListener extends ProtocolListener implements
		Runnable {
	...
	public ProtocolIMDiskListener(Storage storage, MirrorManager mirrorMan,
			SnapshotManager snapMan, LunManagement lm, int lun, String adapter,
			int port, String name, ZoneManager zm, long size,
			boolean assumeBarriers) {
		this.lm = lm;
		this.lun = lun;
		...
		Log.log(LogLevel.LOG_INFO, "Starting IMDisk protocol listener for lun "
	...
	public String getConfigLine() {
		return lun + " imdisk " + adapter + " " + port + " " + name + " "
				+ assumeBarriers;
	}
		...
		for (;;) {
			try {
				...
				if (zm.isAllowed(lun, mcs.getSocket())) {
					...
					ProtocolIMDisk handler = new ProtocolIMDisk(
							lm.getSectorMapper(), lun, storage, mirrorMan,
							snapMan, mcs, size, assumeBarriers);
					Thread t = new Thread(handler, "IMDisk lun " + name + "/"
							+ lun);
					t.start();
