...
public enum LogLevel {
	LOG_EMERG(0, "system is unusable"), LOG_ALERT(1,
			"action must be taken immediately"), LOG_CRIT(2,
			"critical conditions"), LOG_ERR(3, "error conditions"), LOG_WARN(4,
			"warning conditions"), LOG_NOTICE(5,
			"normal, but significant, condition"), LOG_INFO(6,
			"informational message"), LOG_DEBUG(7, "debug-level message");
	...
	public static LogLevel getLogLevel(int level) {
		LogLevel[] levels = new LogLevel[] { LOG_EMERG, LOG_ALERT, LOG_CRIT,
				LOG_ERR, LOG_WARN, LOG_NOTICE, LOG_INFO, LOG_DEBUG };
		if (level > 7) {
