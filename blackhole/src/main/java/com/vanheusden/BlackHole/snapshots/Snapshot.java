...
public class Snapshot implements Runnable {
	...
	public Snapshot(String mmJournalBitmap, long lunSize, int blockSize,
			Storage storageCallBack, SectorMapper sm, String snapshotFile,
			int lun, EncryptionParameters ep) throws IOException, VHException {
	...
	public void queue(long offset) throws IOException, VHException,
		...
		if (offset < snapshotPointer || mmjb.getBit(blockNr) == true) {
			...
			byte[] blData = sp.readBlock(sm.getMapOffset(lun, offset)
					/ blockSize);
			if (!all0x00(blData)) {
		...
		for (;;) {
			try {
				...
				if (snapshotPointer == lunSize) {
					...
					finished = true;
					unlock();
					Log.log(LogLevel.LOG_INFO, "Snapshot " + snapshotFile
							+ " finished");
					break;
				}
				...
				if (mmjb.getBit(curBlockNr) == false) {
					byte[] blData = sp.readBlock(sm.getMapOffset(lun,
					...
					if (encryptorWrite != null) {
						try {
							...
							throw new VHException("Crypto exception: "
									+ pbe.toString());
				...
				if (progress != lastProgress) {
					...
					long MBps = (long) ((snapshotPointer / (1024.0 * 1024.0)) / Math
							.max(1.0, runTime));
					int eta = (int) Math.ceil((bytesLeft / (1024.0 * 1024.0))
							/ (double) MBps);
					Log.log(LogLevel.LOG_INFO, "Snapshot " + snapshotFile
							+ " progress: " + (bytesLeft / blockSize)
							+ " blocks left (" + (bytesLeft / (1024 * 1024))
							+ "MB), " + MBps + "MB/s, run time: " + runTime
							+ "s, eta: " + eta + "s");
