...
public class MetroMirror_iscsi extends MetroMirror implements Runnable {
	...
	public MetroMirror_iscsi(String host, int port, String targetName,
			String initiatorName, String mmJournalBitmap, int lun,
			long backendSize, int blockSize, boolean disconnectedBlock,
			SectorMapper sm) throws IOException {
		super(host, port, mmJournalBitmap, lun, backendSize, blockSize,
				disconnectedBlock, sm);
	...
	void initSession() throws IOException {
		...
		if (pduIn.getByte(3) != 0) {
			Log.log(LogLevel.LOG_WARN, "Failed retrieving capacity of LUN "
					+ targetName
	...
	public boolean transmitBlockLow(long offset, byte[] data)
			throws IOException {
		try {
			// write block
			...
			Log.log(LogLevel.LOG_WARN,
					"iMM transmitBlockLow() socket exception " + se);
		} catch (IOException ie) {
			Log.log(LogLevel.LOG_WARN, "iMM transmitBlockLow() IO exception "
					+ ie);
		}
