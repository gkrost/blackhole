...
public class GlobalMirror extends Mirror implements Runnable {
	...
	public GlobalMirror(String host, int port, String mmJournalBitmap, int lun,
	...
	void reconnect() throws IOException, VHException {
		...
		try {
			if (discNotified == false)
				Log.log(LogLevel.LOG_DEBUG, "GM reconnecting to " + host + ":"
						+ port);
			if (s != null)
				s.close();
			...
			Log.log(LogLevel.LOG_INFO,
					"GM connected (local port: " + s.getLocalPort() + ")");
			...
			if (!discNotified) {
				Log.log(LogLevel.LOG_WARN, "GM reconnect() connect exception "
						+ ce);
				discNotified = true;
			}
		} catch (SocketException se) {
			if (!discNotified) {
				Log.log(LogLevel.LOG_WARN, "GM reconnect() socket exception "
						+ se);
				discNotified = true;
			}
		} catch (IOException ie) {
			if (!discNotified) {
				Log.log(LogLevel.LOG_WARN, "GM reconnect() IO exception " + ie);
				discNotified = true;
			}
		}
	...
	private void doFlushBacklog() throws IOException, VHException,
		...
		for (;;) {
			...
			byte[] data = sp.readBlock(sm.getMapOffset(lun, usedPage
					* blockSize)
					/ blockSize);
		...
		if (nFailed > 0)
			Log.log(LogLevel.LOG_WARN, "GM failed to transmit " + nFailed
					+ " pages. will be retried");
	}

	private void doOneBlock(long blockId) throws IOException, VHException,
			SQLException, DataFormatException, BadPaddingException,
			IllegalBlockSizeException {
		...
		byte[] data = sp.readBlock(sm.getMapOffset(lun, blockId * blockSize)
				/ blockSize);
	...
	public void close() throws IOException, VHException, SQLException,
			DataFormatException, BadPaddingException, IllegalBlockSizeException {
		resyncSem.acquireUninterruptibly();
		if (resync == true)
			Log.log(LogLevel.LOG_WARN, "GM WARNING: not finished syncing! "
		...
		if (mmjb.findBit1(true, totalNBlocks) != -1)
			Log.log(LogLevel.LOG_WARN,
					"GM WARNING some blocks were not transmitted after disconnection! ("
							+ mmjb.getNBits1() + ")");
	...
	public void run() {
		...
		for (;;) {
			try {
				...
				if (!resync) {
					...
					if (progress != resyncLastProgress) {
						...
						long MBps = (long) ((resyncOffset / (1024.0 * 1024.0)) / Math
								.max(1, runTime));
						int eta = (int) Math
								.ceil((bytesLeft / (1024.0 * 1024.0))
										/ (double) MBps);
						Log.log(LogLevel.LOG_INFO, "GM resync progress: "
								+ (bytesLeft / blockSize) + " blocks left ("
								+ (bytesLeft / (1024 * 1024)) + "MB), " + MBps
								+ "MB/s, run time: " + runTime + "s, eta: "
								+ eta + "s");
				...
				Log.log(LogLevel.LOG_WARN, "GM run() socket exception " + se);
			} catch (IOException ie) {
				Log.log(LogLevel.LOG_WARN, "GM run() IO exception " + ie);
		...
		if (s != null && s.isConnected()) {
			try {
				...
				if (returnedHandle != handle)
					throw new VHException("Expected handle " + handle
							+ ", got " + returnedHandle);
