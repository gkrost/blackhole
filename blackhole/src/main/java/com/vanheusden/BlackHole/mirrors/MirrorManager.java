...
public class MirrorManager {
	...
	public MirrorManager(SectorMapper sm, Storage s, String dataPath,
			int blockSize) {
	...
	public void queue(int lun, long offset, long len) throws Exception {
		...
		try {
			for (Mirror mirror : mirrors) {
				// Log.log(LogLevel.LOG_DEBUG, "mm " + offset + " " +
				// data.length);
				if (mirror.getLun() == lun) {
					...
					for (long index = startOffset; index < endOffset; index += blockSize) {
						byte[] data = storage.readBlock(sm.getMapOffset(lun,
								index) / blockSize);
	...
	public void addMirror(MirrorParameters mp) throws VHException,
			SQLException, IOException, DataFormatException,
		...
		if (mp.getMirrorType() == MirrorType.METRO_MIRROR) {
			Log.log(LogLevel.LOG_INFO, "Added sync mirror (metro mirror) to "
					+ to);
			if (mp.getProtocolType() == ProtocolType.PROTOCOL_ISCSI)
				mirror = new MetroMirror_iscsi(mp.getHost(), mp.getPort(),
						mp.getPar1(), mp.getPar2(), dataPath
								+ "/mm-journal-bitmap" + subFileName + ".dat",
						mp.getLun(), lunSize, blockSize,
						mp.getDisconnectBlock(), sm);
			else
				mirror = new MetroMirror(mp.getHost(), mp.getPort(), dataPath
						+ "/mm-journal-bitmap" + subFileName + ".dat",
						mp.getLun(), lunSize, blockSize,
						mp.getDisconnectBlock(), sm);
		} else if (mp.getMirrorType() == MirrorType.GLOBAL_MIRROR) {
			Log.log(LogLevel.LOG_INFO,
					"Added a-sync mirror (global mirror) to " + to);
			if (mp.getProtocolType() == ProtocolType.PROTOCOL_ISCSI) {
				mirror = new GlobalMirror_iscsi(mp.getHost(), mp.getPort(),
						mp.getPar1(), mp.getPar2(), dataPath
								+ "/gm-journal-bitmap" + subFileName + ".dat",
						mp.getLun(), lunSize, blockSize, sm);
			} else
				mirror = new GlobalMirror(mp.getHost(), mp.getPort(), dataPath
						+ "/gm-journal-bitmap" + subFileName + ".dat",
						mp.getLun(), lunSize, blockSize, sm);
		} else {
		...
		if (mp.getMirrorType() == MirrorType.GLOBAL_MIRROR) {
			...
			Thread mirrorThread = new Thread(mirror, "mirror "
					+ mp.getMirrorType() + "/" + to);
			mirrorThread.start();
	...
	private void addMirror(Mirror mirror) throws IOException, VHException,
	...
	public void flushMirrors() throws IOException, VHException, SQLException,
			DataFormatException, BadPaddingException, IllegalBlockSizeException {
		readLock();
