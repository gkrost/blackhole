...
public class ThreadLunReaper implements Runnable {
	...
	public void killLun(int lunNr) {
		...
		while (index < list.size()) {
			if (list.get(index).getLun() == lunNr
					&& list.get(index).getEndPoint() != null) {
	...
	public void shutdown() {
		...
		for (ThreadLun cur : list) {
			Log.log(LogLevel.LOG_INFO,
					" stopping: " + cur.getLun() + " " + cur.getName() + " / "
							+ cur.getEndPoint());
			try {
			...
			for (;;) {
				...
				while (index < list.size()) {
					...
					if (currentThread.isAlive() == false) {
						Log.log(LogLevel.LOG_DEBUG, "Thread "
								+ list.get(index).getName() + " forgotten");
						list.remove(index);
					} else {
						index++;
					}
