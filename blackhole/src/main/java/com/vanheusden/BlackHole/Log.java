...
public class Log {
	...
	static LogLevel debugLevel = LogLevel.LOG_DEBUG,
			logDebugLevel = LogLevel.LOG_DEBUG;
	...
	static public void showAssertionError(AssertionError e) {
		...
		Log.log(LogLevel.LOG_CRIT, "Thread: "
		...
		for (StackTraceElement ste : e.getStackTrace()) {
			Log.log(LogLevel.LOG_CRIT,
					" "
							...
							+ ste.getMethodName()
							+ ", "
							+ (ste.isNativeMethod() ? "is native method"
									: "NOT a native method"));
		}
	}
	...
	public static void sendViaSyslog(LogLevel ll, String what) {
		if (syslogHost != null) {
			...
			try {
				...
				System.err.println("Syslog: unknown host " + syslogHost + ": "
						+ uhe);
			} catch (SocketException se) {
				System.err.println("Syslog: socket exception: " + se);
			} catch (IOException ie) {
				System.err.println("Syslog: I/O exception: " + ie);
			}
		}
	}
