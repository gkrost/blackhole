...
import com.vanheusden.BlackHole.sockets.MyHTTPServer;
...
public class HTTPServer implements Runnable {
	...
	public HTTPServer(String adapter, int port,
			CopyOnWriteArrayList<Thread> threads, Storage storage) {
	...
	public void sendReply_cgibin_statistics_cgi(MyHTTPServer socket)
			throws IOException {
		List<String> reply = new ArrayList<String>();
		...
		try {
			reply.add("<TABLE>\n");
			reply.add("<TR><TD>Total running time:</TD><TD>"
					+ ((double) (System.currentTimeMillis() - BlackHole.runningSince) / 1000.0)
					+ "s</TD></TR>\n");
			reply.add("<TR><TD>Number of webserver hits:</TD><TD>"
					+ webServerHits + "</TD></TR>\n");
			reply.add("<TR><TD>Number of 404 pages serverd:</TD><TD>"
					+ webServer404 + "</TD></TR>\n");
			reply.add("</TABLE>\n");
			reply.add("<BR>\n");
			reply.add("<TABLE>\n");
			...
			reply.add("<TR><TD>System load:</TD><TD>"
					+ osmxb.getSystemLoadAverage() + "</TD></TR>\n");
			ThreadMXBean tmxb = ManagementFactory.getThreadMXBean();
			reply.add("<TR><TD>CPU time used:</TD><TD>"
					+ (tmxb.getCurrentThreadCpuTime() / 1000000000)
					+ "s</TD></TR>\n");
			reply.add("</TABLE>\n");
			reply.add("<BR>\n");
			// reply.add("<TABLE>\n");
			// reply.add("<TR><TD>% gained:</TD><TD>" +
			// (storage.getPercentageBlockUseDecrease()) + "%</TD></TR>\n");
			// reply.add("</TABLE>\n");
			// reply.add("<BR>\n");
			// double avg = (double)TreeOnDisk.totalNSearchLengths.get() /
			// (double)TreeOnDisk.nSearches.get();
			// double sd = Math.sqrt((TreeOnDisk.stddevLengths.get() /
			// (double)TreeOnDisk.nSearches.get()) - Math.pow(avg, 2.0));
			// reply.add("<TR><TD>Stddev tree depth:</TD><TD>" + sd +
			// "</TD></TR>\n");
	...
	public void sendReply_cgibin_stacktraces_cgi(MyHTTPServer socket)
			throws IOException {
		List<String> reply = new ArrayList<String>();
		...
		try {
			...
			while (it.hasNext()) {
				...
				reply.add("<TR><TH COLSPAN=2>" + current.getName()
						+ "</TH><TH COLSPAN=2>" + current.getId()
						+ "</TH></TR>\n");
				StackTraceElement[] ste = traces.get(current);
				for (StackTraceElement currentSte : ste) {
					reply.add("<TR><TD>" + currentSte.getClassName()
							+ "</TD><TD>" + currentSte.getFileName()
							+ "</TD><TD>" + currentSte.getLineNumber()
							+ "</TD><TD>" + currentSte.getMethodName()
							+ "</TD></TR>\n");
				}
			}
	...
	public void sendReply_cgibin_locks_cgi(MyHTTPServer socket)
			throws IOException {
		List<String> reply = new ArrayList<String>();
		...
		try {
			...
			ThreadInfo[] tis = bean.getThreadInfo(bean.getAllThreadIds(), true,
					true);
			for (ThreadInfo ti : tis) {
				reply.add("<TR><TD>" + ti.getLockName() + "</TD><TD>"
						+ ti.getLockOwnerName() + "</TD><TD>"
						+ ti.getThreadName() + "</TD><TD>" + ti.getLockInfo()
						+ "</TD></TR>\n");
			}
	...
	public void sendReply_404(MyHTTPServer socket, String url)
			throws IOException {
		List<String> reply = new ArrayList<String>();
	...
	public void run() {
		...
		try {
			...
			Log.log(LogLevel.LOG_CRIT,
					"HTTPServer Cannot create listen socket: " + e);
			System.exit(127);
		}
	}
}
