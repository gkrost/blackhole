...
import com.vanheusden.BlackHole.cache.CacheClearType;
...
import com.vanheusden.BlackHole.mirrors.MirrorType;
import com.vanheusden.BlackHole.protocol.ProtocolType;
import com.vanheusden.BlackHole.storage.CompressionParameters;
...
public class Config {
	...
	String encryptionPassword = null,
			encryptionCipher = "Blowfish/CFB8/NoPadding";
	...
	static MirrorParameters parseMirrorString(String data) throws VHException {
		...
		if (parts.length != 6 && parts.length != 8)
			throw new VHException(
					"Mirror line not 6 (or 8 for iSCSI) parameters");
		...
		if (parts[1].equalsIgnoreCase("sync"))
			...
			throw new VHException(
					"Mirror type can be either sync or async, not " + parts[1]);
		...
		if (parts[2].equalsIgnoreCase("NBD"))
			...
			throw new VHException(
					"Protocol type can be only iSCSI or NBD currently, not "
							+ parts[2]);
		...
		return new MirrorParameters(mt, pt, host, port, disconnectBlock, par1,
				par2, lun);
	}
	...
	public void saveConfig(String whereTo, Storage s, MirrorManager mirrorMan)
			throws IOException {
		if (whereTo == null)
		...
		for (MirrorParameters m : mirrorMan.getMirrorPars()) {
			write(out, "mirror=" + m.getMirrorType().toString() + " "
					+ m.getProtocolType().toString() + " " + m.getHost() + " "
					+ m.getPort() + " " + m.getDisconnectBlock());
		}
		...
		try {
			...
			try {
				while ((line = in.readLine()) != null) {
					...
					int is = line.indexOf("=");
					if (is == -1)
						throw new VHException("Error on line " + lineNr
								+ ": malformed line.");

					String name = line.substring(0, is).trim();
					String data = line.substring(is + 1).trim();

					boolean isTrue = data.equalsIgnoreCase("true") ? true
							: false;
					...
					if (name.equals("block-size"))
						...
						if (l == null)
							logDebugLevel = LogLevel.getLogLevel(Integer
									.valueOf(data));
						else
							logDebugLevel = l;
						...
						LogLevel l = LogLevel.getLogLevel(data);
						if (l == null)
							debugLevel = LogLevel.getLogLevel(Integer
									.valueOf(data));
						else
							debugLevel = l;
						...
						if (hashType == null) {
							System.err.println("hash-type \"" + data
									+ "\" not known");
							System.exit(1);
						}
						...
						if (cct == null) {
							System.err.println("cache-type \"" + data
									+ "\" not known");
							System.exit(1);
						}
						...
						if (storageType == null) {
							System.err.println("storage-type \"" + data
									+ "\" not known");
							System.exit(1);
						}
					} else
						throw new VHException("Unknown parameter on line "
								+ lineNr);
				}
			} catch (ArrayIndexOutOfBoundsException aioobe) {
				System.err.println("Please check line " + lineNr
				...
				System.err.println("Please check line " + lineNr
						+ " of configuration-file " + fileName
						+ ": one of the parameters ought to be a number");
				System.exit(127);
			}
	...
	public WritecacheParameters getAllWritecacheParameters() {
		...
		return new WritecacheParameters(forceFlushThreshold,
				triggerFlushThreshold, sortWriteBackListBeforeWrite,
				delayedWriteFlushInterval, minFlushN, readCacheDirtyBlocks);
	}
