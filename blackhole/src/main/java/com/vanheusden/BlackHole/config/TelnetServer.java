...
import com.vanheusden.BlackHole.auth.LDAP;
import com.vanheusden.BlackHole.auth.Users;
import com.vanheusden.BlackHole.mirrors.Mirror;
import com.vanheusden.BlackHole.mirrors.MirrorManager;
import com.vanheusden.BlackHole.protocol.ProtocolType;
import com.vanheusden.BlackHole.snapshots.Snapshot;
...
public class TelnetServer implements Runnable {
	...
	class TelnetServerClient implements Runnable {
		...
		TelnetServerClient(Socket cs, Storage sp, Config c, LunManagement lm,
				MirrorManager mm, SnapshotManager sm, ZoneManager zm)
				throws IOException {
			this.cs = cs;
			this.sp = sp;
			this.c = c;
		...
		public String readLine(boolean visible, int maxLen) throws IOException,
				VHException {
			String str = "";
		...
		public void help(boolean useLDAP) throws IOException {
			...
			send("\r\n");
			send("df               show diskspace of "
					+ config.getDatastorePath()
					+ ". Please note that this doesn't work with the MySQL backend.\r\n");
			send("\r\n");
		...
		public void run() {
			...
			try {
				...
				if (config.getLDAPUrl() != null
						&& config.getLDAPBaseDN() != null)
					verifyLDAP = true;
				...
				if (verifyLDAP)
					validUser = LDAP.authenticateUser(config.getLDAPBaseDN(),
							username, password, config.getLDAPUrl());
				else if (users != null)
				...
				for (; validUser;) {
					...
					if (parts[0].equalsIgnoreCase("snapshot")) {
						if (parts.length != 3)
							...
							sm.startSnapshot(Integer.valueOf(parts[1]),
									parts[2], null);
						...
						if (parts.length != 5)
							...
							sm.startSnapshot(Integer.valueOf(parts[1]),
									parts[2], new EncryptionParameters(
											parts[3], parts[4]));

							send("Encrypted snapshot queued for lun "
									+ parts[1] + "\r\n");
						}
						...
						send("Space on " + config.getDatastorePath() + ": "
								+ ((f.getUsableSpace() + GB - 1) / GB)
								+ "GB\r\n");
						...
						send("Number of running snapshots: "
								+ nRunningSnapshots + "\r\n");
					} else if (parts[0].equalsIgnoreCase("listds")) {
						send("Datastore size: "
								+ ((storage.getSize() + GB - 1) / GB) + "GB ("
								+ storage.getSize() + " bytes)\r\n");
						...
						config.saveConfig(config.getConfigFilename(), storage,
								mm);
						...
						for (Snapshot current : sm.getRunningSnapshots())
							send(" " + current.getFilename() + ": "
									+ (current.getBytesLeft() / (1024 * 1024))
									+ "MB left\r\n");
						...
						for (ThreadLun current : curList) {
							...
							send(" "
									...
									+ current.getName()
									+ " "
									+ current.getProtocolListener()
											.getNetworkAddress()
									+ " "
									+ current.getProtocolListener()
											.getProtocol() + " "
									+ (lunSize / GB) + "GB (" + lunSize
									+ ")\r\n");
						}
						...
						if (parts.length != 5)
							...
							if (lm.verifySpaceAvailable(size)) {
								...
								send("Not enough space in datastore for new lun "
										+ parts[4] + "\r\n");
							}
						}
						...
						if (parts.length != 6)
							...
							if (lm.verifySpaceAvailable(size)) {
								...
								int nr = lm.addLun(ProtocolType.PROTOCOL_NBD,
										new String[] { parts[1], parts[2],
												parts[5] }, size, parts[4]);
								if (nr != -1)
									send("Created new lun with number: " + nr
											+ "\r\n");
							} else {
					...
					} else if (parts[0].equalsIgnoreCase("adduser")
							&& !verifyLDAP) {
						if (parts.length != 2)
					...
					} else if (parts[0].equalsIgnoreCase("passwd")
							&& !verifyLDAP) {
						send("\r\nEnter old password: ");
						...
						else if (users.changePassword(username, passwordo,
								password1) == false)
							send("Failed to change password\r\n");
						else
							send("Password changed");
					} else if (parts[0].equalsIgnoreCase("listusers")
							&& !verifyLDAP) {
						...
						for (String current : uList)
							send(" " + current);
						send("\r\n");
					} else if (parts[0].equalsIgnoreCase("deluser")
							&& !verifyLDAP) {
						if (parts.length != 2)
							send("Incorrect number of parameters [username]\r\n");
						else {
						...
						if (parts.length != 7)
							...
							lm.copyLun(nr, parts[2].equalsIgnoreCase("yes"),
									ProtocolType.PROTOCOL_NBD, new String[] {
						...
						for (Mirror mirror : mm.getMirrors()) {
							...
							double perc = (mirror.getResyncOffset() * 100.0)
									/ beSize;
							send(" "
									+ mirror.toString()
									+ " "
									+ (mirror.isSyncing() ? "syncing (" + perc
											+ "%)" : "") + ", pending blocks: "
									+ mirror.getPendingBlockCount() + "\r\n");
							n++;
						}
						send("--- " + n + " mirror(s)\r\n");
						...
						if (parts.length != 2)
							...
							for (Mirror mirror : mm.getMirrors()) {
								if (mirror.toString().equals(parts[1])) {
									send("Starting resync of mirror "
											+ mirror.toString() + "\r\n");
						...
						if (askYesNo("Are you sure?")) {
							...
							int nRunningSnapshots = sm.getRunningSnapshots()
									.size();
							if (nRunningSnapshots > 0)
								send(""
										+ nRunningSnapshots
										+ " running snapshots were terminated and are incomplete!\r\n");
				...
				Log.log(LogLevel.LOG_WARN, "TS run() socket exception " + se);
			} catch (Exception e) { // OK
				Log.showException(e);

				try {
					send("Closing connection because of exception: " + e);
	...
	public TelnetServer(String adapter, int port, Storage s, Config c,
			LunManagement lm, MirrorManager mm, SnapshotManager sm,
			ZoneManager zm) {
	...
	public void run() {
		...
		Log.log(LogLevel.LOG_INFO, "Telnet listening on " + adapter + ":"
				+ port);
		...
		for (;;) {
			...
			try {
				...
				Log.log(LogLevel.LOG_INFO, "Telnet connection with "
						+ client.getInetAddress().toString());

				Thread clientThread = new Thread(new TelnetServerClient(client,
						storage, config, lm, mm, sm, zm), "telnet client: "
						+ client.toString());
				clientThread.start();
				...
				if (Thread.interrupted()) {
					Log.log(LogLevel.LOG_INFO,
							"TelnetServer thread interrupted");
					break;
				}
			} catch (IOException ioe) {
