...
public class SCSI_write_10 extends SCSI_response {
	...
	public SCSI_write_10(FileChannel fc, CDB cdb, int blockSize, byte[] data)
			throws IOException {
		byte[] bIn = cdb.bytes;
		long lba = Utils.byteArrayToInt(bIn, 2) & 0xffffffff;
		int nBlocks = ((int) (bIn[7] & 0xff) << 8) + (bIn[8] & 0xff);

		System.out.println("   cdb # blocks: " + nBlocks + ", data # blocks: "
				+ (data.length / blockSize));
		if (nBlocks != data.length / blockSize)
			System.err.println("   cdb # blocks: " + nBlocks
					+ ", data # blocks: " + (data.length / blockSize));
