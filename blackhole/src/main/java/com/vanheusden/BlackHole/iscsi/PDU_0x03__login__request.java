...
public class PDU_0x03__login__request extends PDU {
	...
	// mode = 2: login to lun
	// mode = 3: no data
	public PDU_0x03__login__request(int esn, String targetLun, int mode,
			String myIQN) {
		setBit(0, 6, true); // 1
		setBits(0, 0, 0x03, 6); // byte 0, bit 0-5, opcode 0x03, 6 wide
		setBit(1, 7, true); // T=1
		setBit(1, 6, false); // C=0
		setBits(1, 2, 1, 2); // byte 1, bit 2-3, value 1, 2 wide:
								// LoginOperationalNegotiation (CSG: 01)
		...
		bytes[16] = bytes[17] = bytes[18] = bytes[19] = 0; // initiator task tag
		bytes[20] = 0;
		bytes[21] = 0; // CID
		bytes[24] = bytes[25] = bytes[26] = 0;
		bytes[27] = 1; // CmdSN
		...
		if (mode == 1) {
			keyValuePairs = new String[] { "InitiatorName=" + myIQN,
					"SessionType=Discovery", "HeaderDigest=None",
					"DataDigest=None", "DefaultTime2Wait=0",
					"DefaultTime2Retain=0", "IFMarker=No", "OFMarker=No",
					"ErrorRecoveryLevel=0", "MaxRecvDataSegmentLength=32768" };
		} else if (mode == 2) {
			String kvTargetLun = "TargetName=" + targetLun;
			keyValuePairs = new String[] {
					"InitiatorName=" + myIQN,
					// "InitiatorAlias=belle",
					kvTargetLun, "SessionType=Normal", "HeaderDigest=None",
					"DataDigest=None", "DefaultTime2Wait=2",
					"DefaultTime2Retain=0", "IFMarker=No", "OFMarker=No",
					"ErrorRecoveryLevel=0", "InitialR2T=Yes",
					"ImmediateData=Yes", "MaxBurstLength=4096",
					"FirstBurstLength=4096", "MaxOutstandingR2T=1",
					"MaxConnections=1", "DataPDUInOrder=Yes",
					"DataSequenceInOrder=Yes", "MaxRecvDataSegmentLength=4096" };
		}
		if (mode == 1 || mode == 2) {
	...
	public long getISID() {
		return (((long) getInt(8)) << 16) + ((long) (bytes[12] & 0xff) << 8)
				+ (long) (bytes[13] & 0xff);
	}
