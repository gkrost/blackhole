...
public class ReadCache {
	...
	//
	AtomicLong cntAdd = new AtomicLong(), cntGet = new AtomicLong(),
			cntRemove = new AtomicLong(), cntFull = new AtomicLong();
	...
	public ReadCache(ReadcacheParameters rcp, int elementSize, String name) {
		...
		stats.add(new StatsElementCounter(cntRemove, "RC " + name
				+ ": cnt remove"));
		stats.add(new StatsElementAverage(cntFull, cntAdd,
				StatsOutputType.PERCENTAGE, false, "RC " + name
						+ ": % full when add"));
		stats.add(new StatsElementAverage(getHit, cntGet,
				StatsOutputType.PERCENTAGE, false, "RC " + name
						+ ": % hit when get"));
		stats.add(new StatsElementAverage(getMis, cntGet,
				StatsOutputType.PERCENTAGE, false, "RC " + name
						+ ": % mis when get"));
	}
