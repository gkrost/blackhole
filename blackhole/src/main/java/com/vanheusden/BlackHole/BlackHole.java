...
import com.vanheusden.BlackHole.config.Config;
import com.vanheusden.BlackHole.config.HTTPServer;
import com.vanheusden.BlackHole.config.TelnetServer;
import com.vanheusden.BlackHole.debugging.ConcurrencyChecker;
import com.vanheusden.BlackHole.debugging.StackTraceDotter;
import com.vanheusden.BlackHole.iscsi.PDU;
import com.vanheusden.BlackHole.iscsi.PDU_0x03__login__request;
import com.vanheusden.BlackHole.iscsi.PDU_0x04__text__request;
import com.vanheusden.BlackHole.mirrors.MirrorManager;
import com.vanheusden.BlackHole.mirrors.MirrorParameters;
...
import com.vanheusden.BlackHole.storage.StorageFiles;
import com.vanheusden.BlackHole.storage.StorageKC;
import com.vanheusden.BlackHole.storage.StorageMongoDB;
import com.vanheusden.BlackHole.storage.StorageMySQL;
import com.vanheusden.BlackHole.storage.StorageType;
...
public class BlackHole {
	...
	public static void iscsiScanTargets(String host, int port, String myIQN)
			throws IOException {
		try {
			...
			System.out.println("Scanning iSCSI target " + host + " on port "
					+ port + ":");
			...
			pduOut = new PDU_0x04__text__request(1, pduIn.getStatSN() + 1,
					new String[] { "SendTargets=All" });
			outputStream.write(pduOut.getAsByteArray());
			pduIn = PDU.recvPdu(inputStream);
			// display
			byte[] tempBuffer = Arrays.copyOf(pduIn.getData(),
					pduIn.getData().length);
	...
	public static void help() {
		...
		System.out
				.println("--isci-scan-targets host port iqn      Ask an iSCSI target for the LUNs it has");
		System.out.println("--fsck        storage consistency test");
		System.out
				.println("--add-nbd-lun adapter port size name   Add a new lun. Name must not contain spaces");
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			if (args.length == 0)
				System.out
						.println("Invoke program with '--help' to see a list of commandline switches.");
			...
			uncleanPath = config.getDatastorePath() + File.separator
					+ uncleanCheckName;
			...
			if (uncleanFile.exists()) {
				...
				Log.log(LogLevel.LOG_WARN,
						"*** UNCLEAN SHUTDOWN, CHECK FORCED ***");
			}
			uncleanFile.createNewFile();

			Log.log(LogLevel.LOG_INFO, "Storage size: "
					+ (config.getFileSize() / (1024 * 1024))
					+ "MB, block size: " + config.getBlockSize());

			if (config.getStorageType() == StorageType.STORAGE_FILES) {
				if (config.getCompressionParameters() != null)
					throw new VHException(
							"One cannot use compression for 'files' storage backend");
				storage = (Storage) new StorageFiles(config.getFileSize(),
						config.getBlockSize(), config.getDatastorePath(),
						config.getHashType(),
						config.getAllReadcacheParameters(),
						config.getAllWritecacheParameters(),
						config.getEncryptionParameters(), checkDatastore);
			} else if (config.getStorageType() == StorageType.STORAGE_SQL)
				storage = (Storage) new StorageMySQL(config.getFileSize(),
						config.getBlockSize(), config.getDatastorePath(),
						config.getSQLUrl(), config.getSQLUser(),
						config.getSQLPasword(), config.getHashType(),
						config.getAllWritecacheParameters(),
						config.getAllReadcacheParameters(),
						config.getEncryptionParameters(), checkDatastore,
						config.getCompressionParameters());
			else if (config.getStorageType() == StorageType.STORAGE_KC)
				storage = (Storage) new StorageKC(config.getFileSize(),
						config.getBlockSize(), config.getDatastorePath(),
						config.getHashType(),
						config.getAllReadcacheParameters(),
						config.getAllWritecacheParameters(),
				...
				storage = (Storage) new StorageMongoDB(config.getMongoHost(),
						config.getMongoDB(), config.getFileSize(),
						config.getBlockSize(), config.getDatastorePath(),
						config.getHashType(),
						config.getAllReadcacheParameters(),
						config.getAllWritecacheParameters(),
			...
			Log.log(LogLevel.LOG_INFO, "Read cache: each element takes up "
					+ ReadCache.getTotalElementSize() + " bytes");
			...
			if (checkDatastore) {
				...
				if (!storage.fsck()) {
					...
					Log.log(LogLevel.LOG_EMERG,
							"Storage structures seem to be all fine");
					System.out
							.println("Storage structures seem to be all fine");
					if (uncleanStart)
						Log.log(LogLevel.LOG_INFO,
								"It is safe to restart the application now that the datastore is verified");
				}
			...
			if (new File(config.getDatastorePath() + "/sectormap.dat").exists())
				sm = new SectorMapper(config.getFileSize(),
						config.getDatastorePath() + "/sectormap.dat");
			else
				sm = new SectorMapper(config.getFileSize());

			mirrorMan = new MirrorManager(sm, storage,
					config.getDatastorePath(), config.getBlockSize());

			for (MirrorParameters current : config.getMirrors()) {
				mirrorMan.addMirror(current);
			}

			snapMan = new SnapshotManager(config.getSnapshotDir(), storage, sm,
					config.getBlockSize());

			ZoneManager zm = new ZoneManager(config.getDatastorePath()
					+ "/zoning.dat");

			lm = new LunManagement(config.getDatastorePath(), storage,
					mirrorMan, snapMan, sm, config.getDatastorePath()
							+ "/sectormap.dat", zm);
			...
			for (String[] current : nbdLunsToAdd) {
				...
				if (!lm.verifySpaceAvailable(size)) {
					System.err
							.println("not enough space in datastore for new lun "
									+ current[3]);
					System.exit(1);
				}
				if (lm.addLun(ProtocolType.PROTOCOL_NBD, new String[] {
						current[0], current[1] }, size, current[3]) == -1) {
			...
			if (config.getTelnetListenPort() != -1) {
				telnetThread = new Thread(new TelnetServer(
						config.getTelnetListenAdapter(),
						config.getTelnetListenPort(), storage, config, lm,
						mirrorMan, snapMan, zm), "telnet listener");
				telnetThread.start();
			}
			...
			if (config.getHttpListenPort() != -1) {
				Log.log(LogLevel.LOG_DEBUG,
						"Web-server listening on "
								+ config.getHttpListenAdapter() + ":"
								+ config.getHttpListenPort());
				webServerThread = new Thread(new HTTPServer(
						config.getHttpListenAdapter(),
						config.getHttpListenPort(), threadList, storage),
						"HTTP server");
			...
			try {
				...
				Log.log(LogLevel.LOG_INFO,
						"Stopping protocol listeners & instances");
				...
				if (telnetThread != null) {
					Log.log(LogLevel.LOG_INFO,
							"Waiting for telnet server thread to exit");
				...
				if (webServerThread != null) {
					Log.log(LogLevel.LOG_INFO,
							"Waiting for webserver thread to exit");
