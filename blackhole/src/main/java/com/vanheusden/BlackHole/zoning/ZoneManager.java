...
public class ZoneManager {
		...
		try {
			...
			for (;;) {
				...
				if (elements.length != 2)
					throw new VHException("Zoning line " + lineNr
							+ ": invalid line");

				addZone(Integer.valueOf(elements[0]),
						Zone.getInstance(elements[1]));
			}

			fh.close();
		} catch (FileNotFoundException fnfe) {
			Log.log(LogLevel.LOG_WARN, "Zoning: " + file
					+ " not found, first run?");
		}
	}
	...
	public boolean isAllowed(int lun, Socket client)
			throws UnknownHostException {
		sem.acquireUninterruptibly();
		List<Zone> zoneSet = zoning.get(Integer.valueOf(lun));
		if (zoneSet == null || zoneSet.size() == 0) { // empty/non existing
														// zoneset means allow
														// from all
			sem.release();
			return true;
