...
public class MyClientSocket {
	...
	public void getBytes(byte[] out, int offset, int len) throws IOException,
			VHException {
		int totalRead = 0;
		int left = len;
		while (left > 0) {
			...
			if (nRead == -1)
				throw new VHException("read from socket error, expected: "
						+ len + ", got: " + totalRead);
	...
	public int getU32() throws IOException, VHException {
		...
		return ((in[0] & 0xff) << 24) + ((in[1] & 0xff) << 16)
	...
	public void putU32(int data) throws IOException {
		byte[] out = { (byte) (data >> 24), (byte) (data >> 16),
