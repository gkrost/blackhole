...
public class Hasher {
	...
	public static Hasher getInstance(HashType hashType)
		...
		for (int nr = 0; nr < nCPUs; nr++) {
			try {
				...
				Log.log(LogLevel.LOG_DEBUG,
						"Hasher: interrupted while waiting for thread " + nr
								+ " of " + nCPUs);
			}
		}
	...
	class HasherDoit implements Runnable {
		...
		HasherDoit(List<Map.Entry<Long, Object[]>> list, int listSize,
				AtomicInteger pointer, MessageDigest md) {
			this.list = list;
