...
import com.vanheusden.BlackHole.storage.Encryptor;
...
public class TableOnDisk {
	...
	public TableOnDisk(String fileName, int elementSize, long nBlocks,
			EncryptionParameters ep) throws IOException, VHException {
		...
		if (ep != null) {
			try {
				...
				throw new VHException("Crypto exception: " + nsae.toString());
			} catch (NoSuchPaddingException nspe) {
				throw new VHException("Crypto exception: " + nspe.toString());
			} catch (InvalidKeyException ike) {
				throw new VHException("Crypto exception: " + ike.toString());
			} catch (InvalidAlgorithmParameterException iape) {
				throw new VHException("Crypto exception: " + iape.toString());
			}
		}
	...
	void map() {
		...
		try {
			...
			for (int index = 0; index < nMBB; index++) {
				...
				Log.log(LogLevel.LOG_DEBUG, "TOD: creating map " + index
						+ " out of " + nMBB + " for " + fileName);
				mbb[index] = fileChannel.map(FileChannel.MapMode.READ_WRITE,
						offset, Math.min(size - offset, ss));
			}
			// throw new IOException();
		} catch (IOException ie) {
			Log.log(LogLevel.LOG_WARN, "TOD: failed creating (m)map on "
					+ fileName + ", falling back to regular i/o");
	...
	protected void putLow(long blockIndex, byte[] elementIn)
	...
	public void storeElement(long elementNr, byte[] element, int nr)
			throws IOException, VHException {
	...
	public byte[] retrieveElement(long elementNr, boolean completeOnly, int nr)
			throws IOException, VHException {
		byte[] buffer = null;
		int rc = -1;
		long reshuffleNr = -1;
		...
		// Log.log(LogLevel.LOG_ERR, "read failure: " + rc + " " + buffer.length
		// + " " + buffer[0]);
		if (completeOnly)
			return null;
		...
		throw new VHException(
				"TableOnDisk::retrieveElement: short-read while reading element. expecting "
						+ elementSize + ", got " + rc);
	}
