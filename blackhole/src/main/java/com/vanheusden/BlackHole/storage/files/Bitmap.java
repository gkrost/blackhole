...
public class Bitmap {
	...
	private void loadPage(long pageNr) throws IOException {
		...
		} catch (EOFException ee) { // we're reading where we haven't yet stored
									// a bit pattern
			assert false;
		}
	}
	...
	public long findBit0(boolean startAtStart, long maxBitNr)
			throws IOException {
		long node = -1;

		long startAt = startAtStart ? 0
				: (nextSearchStart0 & (~((pageSize * 8) - 1)));
		int pageStart = (int) (nextSearchStart0 & ((pageSize * 8) - 1)) / 8;
		finished2: for (long bitNr = startAt; bitNr < maxBitNr; bitNr += (pageSize * 8)) {
	...
	public long findBit1(boolean startAtStart, long maxBitNr)
			throws IOException {
		long node = -1;

		long startAt = startAtStart ? 0
				: (nextSearchStart1 & (~((pageSize * 8) - 1)));
