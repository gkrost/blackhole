...
import com.vanheusden.BlackHole.cache.UniqueOriginalOrderList;
...
public abstract class DelayedWriteCache implements Runnable {
	...
	public void init() {
		...
		stats.add(new StatsElementCounter(nQueueFull, "DWC " + file
				+ ": queue full"));
		stats.add(new StatsElementCounter(fullTime, "DWC " + file
				+ ": total full time (ms)"));
		stats.add(new StatsElementAverage(fullTime, nQueueFull,
				StatsOutputType.FLOAT, false, "DWC " + file
						+ ": average queue full time (ms)"));
		stats.add(new StatsElementAverage(totalFill, nMeasures,
				StatsOutputType.FLOAT, false, "DWC " + file
						+ ": average number of blocks in cache"));
		stats.add(new StatsElementCounter(nFlushed, "DWC " + file
				+ ": number of flushes"));
		stats.add(new StatsElementAverage(totalSortTime, nFlushed,
				StatsOutputType.FLOAT, false, "DWC " + file
						+ ": average sort time (ms)"));
		stats.add(new StatsElementAverage(totalWriteTime, nFlushed,
				StatsOutputType.FLOAT, false, "DWC " + file
						+ ": average write time (ms) per block"));
		stats.add(new StatsElementAverage(nFlushed, totalWriteTime,
				StatsOutputType.FLOAT, false, "DWC " + file
						+ ": average number of blocks per ms"));
		stats.add(new StatsElementAverage(nFlushedBlocks, nFlushed,
				StatsOutputType.FLOAT, false, "DWC " + file
						+ ": average number of blocks per write (ms)"));
	}

	public boolean[] checkFlush() throws InterruptedException {
		assert checkReadLocked() || checkWriteLocked();
		boolean triggered = delayedWriteList.size() >= wcp
				.getTriggerFlushThreshhold();
		boolean needFlush = delayedWriteList.size() >= wcp
				.getForceFlushThreshold();

		if (needFlush) {
			if (didFlush) {
				Log.log(LogLevel.LOG_INFO,
						what + " force flush start threshold "
								+ delayedWriteList.size() + " "
								+ wcp.getForceFlushThreshold());
				didFlush = false;
			}
			doNotifyAPut();
		} else if (triggered) {
			if (didFlush) {
				Log.log(LogLevel.LOG_INFO,
						what + " flush start threshold "
								+ delayedWriteList.size() + " "
								+ wcp.getTriggerFlushThreshhold());
				didFlush = false;
			}
			doNotifyAPut();
		}
	...
	public void forceFlush() throws InterruptedException {
		...
		Log.log(LogLevel.LOG_INFO, what + " full " + delayedWriteList.size()
				+ " " + wcp.getForceFlushThreshold());
		...
		Log.log(LogLevel.LOG_INFO, what + " full ok " + delayedWriteList.size()
				+ " " + wcp.getForceFlushThreshold());
	}

	public void queueDirtyBlock(long elementNr, byte[] element)
			throws InterruptedException {
		assert checkWriteLocked();

		assert element != null;
		delayedWriteList.put(Long.valueOf(elementNr), new Object[] { element,
				null });
	}
	...
	abstract protected void putLow(long blockNr, byte[] data, byte[] hash)
	...
	private void writeCacheToDisk(boolean forceWriteAll, boolean emergency)
		...
		long timeToNextFlush = wcp.getDelayedWriteFlushInterval()
				- timeSinceLastWrite;
		...
		double timeFactor = Math.min(
				1.01,
				timeSinceLastWrite
						/ (double) wcp.getDelayedWriteFlushInterval());

		tempAvgN = Math.max(wcp.getMinFlushN(), (int) (tempAvgN * timeFactor));

		int nToDo = Math.min(tempAvgN, nAvailable);
		if (emergency)
			tempAvgN = nToDo = (wcp.getTriggerFlushThreshhold() + wcp
					.getForceFlushThreshold()) / 3;
		if (forceWriteAll)
			tempAvgN = nToDo = nAvailable;

		double curCanDo = 0.0;
		if (totalWriteTime.get() > 0) {
			curCanDo = (double) nFlushedBlocks.get()
					/ (double) totalWriteTime.get();
			if (canDo < 0)
				canDo = curCanDo;
			else
		...
		if (nAvailable > wcp.getForceFlushThreshold())
			flushMore = true;
		if (forceWriteAll || flushMore
				|| timeSinceLastWrite > wcp.getDelayedWriteFlushInterval()
				|| emergency) {
			if (nToDo > 0) {
				didFlush = true;
				Log.log(LogLevel.LOG_INFO, "DWC: flush " + nToDo
						+ " blocks of " + nAvailable + ", "
						+ (forceWriteAll ? "forced, " : "") + "avgN: " + avgN
						+ ", time factor: " + timeFactor + ", added: "
						+ (nAvailable - lastN) + ", can do: "
						+ (canDo * timeToNextFlush));
				avgN = nAvailable;

				List<Map.Entry<Long, Object[]>> kvlist = delayedWriteList
						.getFromBottomAndRemove(nToDo);
				...
				if (wcp.getSortWriteBackListBeforeWrite()) {
					class compare implements
							Comparator<Map.Entry<Long, Object[]>> {
						public int compare(Map.Entry<Long, Object[]> a,
								Map.Entry<Long, Object[]> b) {
						...
						public boolean equals(Map.Entry<Long, Object[]> a,
								Map.Entry<Long, Object[]> b) {
					...
					totalSortTime.addAndGet(System.currentTimeMillis()
							- sortStart);
				}
				...
				for (int processLoop = 0; processLoop < nToDo; processLoop++) {
					...
					try {
						...
						Log.log(LogLevel.LOG_EMERG,
								"DWC Exception in delayed write thread! ("
										+ what + ") " + e);
						Log.log(LogLevel.LOG_EMERG,
								"DWC data will be re-queued");
						Log.showException(e);

						try {
							queueDirtyBlock(kv.getKey(),
									(byte[]) kv.getValue()[0]);
						} catch (InterruptedException ie) {
							Log.log(LogLevel.LOG_EMERG,
									"DWC failed to re-queue data after exception during put: data is lost! "
											+ ie);
		...
		for (;;) {
			...
			if (!flushMore && !forceWriteAll && !fwa) {
				try {
					...
					Log.log(LogLevel.LOG_INFO,
							"DWC Delayed write sleep interrupted (" + what
									+ ")");
					forceWriteAll = true;
				}
			} else {
				// Log.log(LogLevel.LOG_INFO, "DWC " + flushMore + " " +
				// forceWriteAll);
			}
			...
			try {
				...
				Log.log(LogLevel.LOG_EMERG,
						"DWC flush thread threw exception: " + e);
				Log.showException(e);
			}
			...
			if (exitAfterFlush.get()) {
				Log.log(LogLevel.LOG_INFO, "DWC " + what + " exit after flush "
						+ nBlocks);
				if (nBlocks == 0)
					break;
	...
	public void close() {
		int nAvailable = delayedWriteList.size();
		if (nAvailable > 0) {
			Log.log(LogLevel.LOG_INFO, "DWC " + what
					+ ": please wait while flushing delayed write cache ("
					+ nAvailable + " blocks)");
		}
		...
		Log.log(LogLevel.LOG_INFO, "DelayedWriteCache thread ended with "
				+ delayedWriteList.size() + " blocks left");
	}
}
