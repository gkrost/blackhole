...
public class Encryptor {
	...
	public Encryptor(EncryptionParameters ep) throws NoSuchAlgorithmException,
			NoSuchPaddingException, InvalidKeyException,
			InvalidAlgorithmParameterException {
		SecretKey secretkey = new SecretKeySpec(ep.getPassword().getBytes(),
				ep.getCipher());

		IvParameterSpec ivs = new IvParameterSpec(new byte[] { 0, 1, 2, 3, 4,
				5, 6, 7 });
		...
		stats.add(new StatsElementAverage(encTook, encCnt,
				StatsOutputType.FLOAT, false, "Encryptor " + ep.getCipher()
						+ ": average time per encryption"));
		stats.add(new StatsElementAverage(decTook, decCnt,
				StatsOutputType.FLOAT, false, "Encryptor " + ep.getCipher()
						+ ": average time per decryption"));
		stats.add(new StatsElementCounter(encCnt, "Encryptor " + ep.getCipher()
				+ ": number of encryptions"));
		stats.add(new StatsElementCounter(decCnt, "Encryptor " + ep.getCipher()
				+ ": number of decryptions"));
	}

	public byte[] encrypt(byte[] in) throws BadPaddingException,
	...
	public byte[] decrypt(byte[] in) throws BadPaddingException,
			IllegalBlockSizeException {
		byte[] out;
