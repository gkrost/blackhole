...
public class StorageMongoDB extends Storage {
	...
	public StorageMongoDB(String mongoHost, String mongoDb, long fileSize,
			int blockSize, String dataPath, HashType hashType,
			ReadcacheParameters rcp, WritecacheParameters wcp,
			EncryptionParameters ep, boolean isFsck,
			CompressionParameters compression) throws NoSuchAlgorithmException,
			NoSuchPaddingException, InvalidKeyException,
			InvalidAlgorithmParameterException, NoSuchAlgorithmException,
			IOException, VHException, SQLException {
	...
	void storeHashDataPair(byte[] hash, byte[] data) throws VHException {
		...
		/*
		 * if (rc.getN() != 1) { // FIXME error Log.log(LogLevel.LOG_WARN,
		 * "Failed adding record: " + rc);
		 * 
		 * try { if (getDataByHash(hash) == null) Log.log(LogLevel.LOG_WARN,
		 * "data was not stored"); } catch(Exception e) {
	...
	void mapSectorToHash(long sectorNr, byte[] hash) throws VHException {
		...
		BasicDBObject document = new BasicDBObject("$set", new BasicDBObject(
				"hash", hash));
		...
		if (rc.getN() == 0) {
			...
			Log.log(LogLevel.LOG_WARN,
					"Expecting to automatically create a new sector mapping record but none was processed? sector: "
							+ sectorNr + ", rc: " + rc);
		}
	}
	...
	void updateCountAndDelete(byte[] hash, long count) {
		if (count == 0) {
			...
			if (rc.getN() != 1) {
				// FIXME error
				Log.log(LogLevel.LOG_WARN, "Failed removing data/hash pair: "
						+ rc);
			}
		} else {
			BasicDBObject search = new BasicDBObject();
			...
			BasicDBObject document = new BasicDBObject("$set",
			...
			if (rc.getN() == 0) {
				// FIXME failure
				Log.log(LogLevel.LOG_WARN,
						"Expecting to upgrade count in an existing record, none foud "
								+ count + ": " + rc);
			}
		}
	}
	...
	void putBlockLow(long sectorNr, byte[] data, byte[] newHash)
		...
		if (newSector || replacingData) {
			if (replacingData) {
				...
				if (debug)
					System.out
							.println(" > replacing data, new count of old hash: "
									+ newPrevHashCount);
			...
			if (debug)
				System.out.println(" > new count of new hash: "
						+ newNewHashCount);
	...
	static void compareBlocks(byte[] b1, byte[] b2) {
		...
		for (int index = 0; index < l; index++) {
			if (b1[index] != b2[index]) {
				...
				System.err.println("" + index + "] " + b1[index] + " / "
						+ b2[index]);
			}
		}
	}
	...
	public static void main(String[] args) throws Exception {
		StorageMongoDB s = new StorageMongoDB("localhost", "test",
				16l * 1024 * 1024 * 1024, 4096, null, HashType.MD5, null, null,
				null, false, null);
		s.debug = true;
		...
		System.out
				.println(" * put & get 1 block in empty storage at sector nr 0"); // ***
		...
		if (block_get == null) {
			...
			System.err
					.println(" - Read back block after simple put: contents differ");
			compareBlocks(block, block_get);
			fail = true;
		}

		System.out
				.println(" * verify that the data block can be found via its hash"); // ***
		...
		System.out
				.println(" * write another instance of that block on sector 1, will trigger de-dup"); // ***
		s.putBlockLow(1, block);

		System.out
				.println(" * verify that the usage count of the hash now is 2"); // ***
		...
		System.out
				.println(" * verify that sector 1 can be read and contains valid data"); // ***
		...
		if (block_get == null) {
			...
			System.err
					.println(" - Read back block after dedup: contents differ");
			compareBlocks(block, block_get);
			fail = true;
		}
		...
		System.out
				.println(" * verify that the hash in the sectormap is the same as the one inserted"); // ***
