...
public class TreeOnDisk {
	...
	public TreeOnDisk(String tableFilename, String bitmapFilename,
			String topNodeFilename, long maxNBlocks, int hashSize,
			boolean isFsck) throws IOException, VHException {
		tod = new TableOnDisk(tableFilename,
				hashSize /* hash */+ 8 /* left */+ 8 /* right */, maxNBlocks, null);
	...
	public int findDepth(long start, boolean left) throws IOException,
			VHException {
		long current = start;
		int length = 0;
	...
	public void storeTopNode() throws IOException {
		...
		Log.log(LogLevel.LOG_DEBUG, "TreeOnDisk: use node " + topNode
				+ " as top node");
	}
	...
	public void dumpNode(BufferedWriter out, long p) throws IOException,
			VHException {
		if (p == -1)
			return;

		TreeOnDiskNode todnIn = new TreeOnDiskNode(tod.retrieveElement(p,
				false, 8), hashSize);
	...
	public void dumpNode(long p) throws IOException, VHException {
		...
		System.out.println("value: " + Arrays.toString(value) + ", left: "
				+ left + ", right: " + right);
	}

	public long storeNode(long parent, byte[] hash) throws IOException,
			VHException {
		return storeNode(parent, hash, -1);
	}

	public long storeNode(long parent, byte[] hash, long locationInTod)
		...
		TreeOnDiskNode todnIn = new TreeOnDiskNode(tod.retrieveElement(parent,
				true, 9), hashSize);
	...
	public long storeNode(byte[] hash, long locationInTod) throws IOException,
			VHException {
		nBlocksIn++;
		...
		while (current != -1) {
			TreeOnDiskNode todnIn = new TreeOnDiskNode(tod.retrieveElement(
					current, true, 9), hashSize);
			...
			if (Utils.compareValues(hash, todnIn.getValue()) < 0) {
				if (todnIn.getLeft() == -1) {
					...
					// create new object
					TreeOnDiskNode todnOut = new TreeOnDiskNode(hash, -1, -1,
							hashSize);
					// and store it in the free location
	...
	public byte[] retrieveElement(long nr) throws IOException, VHException {
		TreeOnDiskNode todn = new TreeOnDiskNode(tod.retrieveElement(nr, true,
				11), hashSize);
		return todn.getValue();
	}

	public boolean checkTreeForError(long node, TreeSet<Long> check)
			throws IOException, VHException {
		boolean error = false;

		if (topNode == -1) { // EMPTY datastore
			System.out.println("empty datastore(1) ");
			return false; // FIXME; check that the topnodefile also does not
							// exist
		}
		...
		TreeOnDiskNode todn = new TreeOnDiskNode(tod.retrieveElement(node,
				true, 18), hashSize);
		TreeOnDiskNode tl = null, tr = null;
		if (todn.getLeft() != -1) {
			tl = new TreeOnDiskNode(tod.retrieveElement(todn.getLeft(), true,
					19), hashSize);
		...
		if (todn.getRight() != -1) {
			tr = new TreeOnDiskNode(tod.retrieveElement(todn.getRight(), true,
					20), hashSize);
		...
		if (tl != null
				&& Utils.compareValues(tl.getValue(), todn.getValue()) > 0) {
			Log.log(LogLevel.LOG_EMERG, "left value bigger");
			error = true;
		}
		if (tr != null
				&& Utils.compareValues(tr.getValue(), todn.getValue()) < 0) {
	...
	public long findParent(long child) throws IOException, VHException {
		...
		long found = -1;
		long length = 0;
		TreeOnDiskNode todnChild = new TreeOnDiskNode(tod.retrieveElement(
				child, true, 13), hashSize);

		while (current != -1) {
			length++;
			TreeOnDiskNode todn = new TreeOnDiskNode(tod.retrieveElement(
					current, true, 14), hashSize);
			if (todn.getLeft() == child || todn.getRight() == child) {
				found = current;
				break;
			}

			int diff = Utils.compareValues(todnChild.getValue(),
					todn.getValue());
			if (diff < 0)
	...
	public void updateLeftPointer(long node, long p) throws IOException,
			VHException {
		TreeOnDiskNode todn = new TreeOnDiskNode(tod.retrieveElement(node,
				true, 15), hashSize);
		todn.setLeft(p);
	...
	public void updateRightPointer(long node, long p) throws IOException,
			VHException {
		TreeOnDiskNode todn = new TreeOnDiskNode(tod.retrieveElement(node,
				true, 16), hashSize);
		todn.setRight(p);
	...
	long getSuccessor(long delNode) throws IOException, VHException {
		...
		TreeOnDiskNode todnDelNode = new TreeOnDiskNode(tod.retrieveElement(
				delNode, true, 12), hashSize);
		...
		// if successor not
		TreeOnDiskNode todn = new TreeOnDiskNode(tod.retrieveElement(successor,
				true, 12), hashSize);
		if (successor != todnDelNode.getRight()) { // right child,
			updateLeftPointer(successorParent, todn.getRight()); // successorParent.leftChild
																	// =
																	// successor.rightChild;

			updateRightPointer(successor, todnDelNode.getRight()); // successor.rightChild
																	// =
																	// delNode.rightChild;
		}
	...
	public boolean deleteNode(long node) throws IOException, VHException {
		...
		TreeOnDiskNode todnParent = new TreeOnDiskNode(tod.retrieveElement(
				parent, true, 21), hashSize);
		boolean isLeftChild = todnParent.getLeft() == node;

		TreeOnDiskNode todnNode = new TreeOnDiskNode(tod.retrieveElement(node,
				true, 22), hashSize);
		...
		// two children, so replace with inorder successor
			 ...
			 * 
			 * if (todnParent.getLeft() == node) todnParent.setLeft(minNode);
			 * else todnParent.setRight(minNode);
			 * 
			 * updateLeftPointer(minNode, todnNodeLeft);
			 */

			long successor = getSuccessor(node);
			// no longer as root moves assert node != 0; // should be: root =
			// successor
			...
			updateLeftPointer(successor, todnNodeLeft); // successor.leftChild =
														// current.leftChild;
		}
		...
		b.setBit(node, false);
		assert b.getBit(node) == false;
		// assert b.findBit0(maxNBlocks) != -1; due to setting new search start,
		// first bit will not be found
	...
	public void rotateRight(long node) throws IOException, VHException {
		...
		TreeOnDiskNode todnLeft = new TreeOnDiskNode(tod.retrieveElement(
				nodeLeft, true, 22), hashSize);
		...
		if (nodeParent != -1)
			todnNodeParent = new TreeOnDiskNode(tod.retrieveElement(nodeParent,
					true, 22), hashSize);
	...
	public void rotateLeft(long node) throws IOException, VHException {
		...
		TreeOnDiskNode todnRight = new TreeOnDiskNode(tod.retrieveElement(
				nodeRight, true, 22), hashSize);
