...
public class StatsElementAverage extends StatsElement {
	...
	public StatsElementAverage(AtomicLong counter, AtomicLong n,
			StatsOutputType sot, boolean abbr, String descr) {
	...
	public String toString() {
		...
		if (sot == StatsOutputType.FLOAT)
			return descr
					+ ": "
					+ abbreviate((double) counter.get() / (double) n.get(),
							false);

		if (sot == StatsOutputType.PERCENTAGE)
			return descr + ": "
					+ ((double) (counter.get() * 100.0) / (double) n.get());
