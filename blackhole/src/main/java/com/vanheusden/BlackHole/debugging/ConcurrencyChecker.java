...
public class ConcurrencyChecker implements Runnable {
	...
	public void run() {
		...
		for (;;) {
			// will contain a list of all 'currently running methods in a
			// thread"
			...
			while (it.hasNext()) {
				...
				StackTraceElement[] trace = (StackTraceElement[]) pairs
						.getValue();
				for (int depth = 0; depth < trace.length; depth++) {
					// also an other thread running this method?
					String sOut = trace[depth].getClassName() + ":"
					...
					if (list.containsKey(sOut)) {
						if (currentThread != list.get(sOut)) {
							...
							if (!knownConflicts.containsKey(sOut)) {
								...
								System.err.println("Concurrency check: "
										+ currentThread.getName() + " / "
										+ list.get(sOut).getName() + ": "
										+ sOut);
								System.err.println(" trace for: "
										+ ((Thread) pairs.getKey()).getName());
