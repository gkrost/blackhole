...
public class StackTraceDotter implements Runnable {
	...
	public void run() {
		...
		for (; exit.get() == false;) {
			Map<Thread, StackTraceElement[]> currentStackTraces = Thread
					.getAllStackTraces();
			...
			while (it.hasNext()) {
				...
				StackTraceElement[] stackTrace = (StackTraceElement[]) pairs
						.getValue();
				...
				// calculate hash on stack-trace, to be used for quick checking
				// for duplicates
				int stStringHash = stString.toString().hashCode();

				Map<Integer, StackTraceElement[]> currentThreadTraces = history
						.get(currentThread);
			...
			try {
				Thread.sleep(1 + r.nextInt(500), r.nextInt(1000000));
		...
		// emit
		int nr = 0;
		Iterator<Map.Entry<String, Map<Integer, StackTraceElement[]>>> it = history
				.entrySet().iterator();
		while (it.hasNext()) {
			Map.Entry<String, Map<Integer, StackTraceElement[]>> pairs = (Map.Entry<String, Map<Integer, StackTraceElement[]>>) it
					.next();
			String currentThread = (String) pairs.getKey();
			Map<Integer, StackTraceElement[]> currentThreadTraces = (Map<Integer, StackTraceElement[]>) pairs
					.getValue();
			System.out.println(currentThread);
			try {
				BufferedWriter out = new BufferedWriter(new FileWriter("traces"
						+ nr + ".dot", true));
				writeLine(out, "digraph {");
				Iterator it2 = currentThreadTraces.entrySet().iterator();
				while (it2.hasNext()) {
					Map.Entry pairs2 = (Map.Entry) it2.next();
					StackTraceElement[] trace = (StackTraceElement[]) pairs2
							.getValue();
					for (int index = 1; index < trace.length; index++)
						writeLine(out, "\t\"" + stToString(trace[index - 1])
								+ "\" -> \"" + stToString(trace[index]) + "\"");
				}
				writeLine(out, "}");
				out.close();
